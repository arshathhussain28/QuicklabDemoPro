...
export const getAllData = async (req: Request, res: Response) => {
  try {
    const [salespersons, distributors, machines, demoTypes, kitParameters] =
      await Promise.all([
...
export const createKitParameter = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to create kit parameter" });
  }
};
...
export const deleteKitParameter = async (req: Request, res: Response) => {
  try {
    const id = getParam(req.params.id);
    await prisma.kitParameter.delete({ where: { id } });

    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: "Failed to delete kit parameter" });
  }
};
...
export const updateKitParameter = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to update kit parameter" });
  }
};
